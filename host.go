...
type Host struct {
	...
	Protocols Inventory `json:"inventory,omitempty"`
}

//Inventory allala
type Inventory struct {
	Telnet string `json:"poc_2_phone_a,omitempty"`
	SSH    string `json:"poc_2_phone_b,omitempty"`
}
...
func (api *API) HostsGet(params Params) (zbx Hosts, err error) {
	if _, present := params["output"]; !present {
		params["output"] = "extend"
	}
	_, b, err := api.CallWithError2("host.get", params)
	...
	json.Unmarshal(b, &zbx)
	//fmt.Println(zbx)
	//fmt.Println(response)
	return
}

func (api *API) HostsGet2(params Params) (zbx Result, err error) {
	if _, present := params["output"]; !present {
		params["output"] = "extend"
	}
	_, b, err := api.CallWithError2("host.get", params)
